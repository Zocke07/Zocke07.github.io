...
// --- Case-study contents ---
// The long pages run to nine sections and five thousand words, and there was no
// way to see that shape or move around inside it: a reader who wanted the
// results of a case study scrolled for them, and the header nav on those pages
// points back at the home page rather than at anything on the page itself.
//
// The list is derived from the sections already collected above rather than
// written into twenty files, so it cannot drift out of step with the headings
// it names, and the Chinese twins pay nothing for it: no markup means nothing
// for the parity check to compare and nothing to translate.
//
// It lives in the strip between the 46rem prose measure and the container edge,
// which is empty on every case study but only exists on a wide screen, so the
// stylesheet stops drawing it below 75rem. On a phone the page is a single
// column and the header is already the way out of it.
const caseHero = $(".case-hero");

if (caseHero && sections.length > 2) {
  const zh = root.lang.startsWith("zh");
  const label = zh ? "本頁內容" : "On this page";

  const toc = document.createElement("nav");
  toc.classList.add("case-toc");
  // The visible label is a <p>: a heading here would land between the page's
  // own h1 and its h2s and break the order they are read in.
  toc.setAttribute("aria-label", label);
  const caption = document.createElement("p");
  caption.classList.add("case-toc-title");
  ...
  sections.forEach((section) => {
    ...
    item.append(link);
    list.append(item);
    tocLinks.set(section, link);
  });

  if (tocLinks.size > 2) {
    toc.append(caption, list);
    // After the header rather than at the end of the body, so tab order reaches
    // it where the eye does. The skip link targets #main and still clears it.
    header ? header.after(toc) : document.body.append(toc);

    // The same band and the same document-order tie-break as the nav spy above:
    // two sections can occupy the band at once and callback order is not
    // document order.
    ...
    const tocSpy = new IntersectionObserver((entries) => {
      ...
      const current = [...tocLinks.keys()].find((s) => inBand.has(s));
      tocLinks.forEach((a) => {
        a.classList.remove("active");
        a.removeAttribute("aria-current");
      });
      if (current) {
        const link = tocLinks.get(current);
        link.classList.add("active");
        link.setAttribute("aria-current", "true");
      }
    }, { rootMargin: "-20% 0px -70% 0px" });

    tocLinks.forEach((_, section) => tocSpy.observe(section));
  }
}
...
const revealTargets = document.querySelectorAll(
  ".section-title, .group-title, .about p, .entry, .card, .brief, .skill-group, .languages, .contact .action-row"
);
